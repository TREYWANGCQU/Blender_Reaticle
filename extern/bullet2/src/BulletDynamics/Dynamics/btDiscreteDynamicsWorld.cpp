...
void	btDiscreteDynamicsWorld::synchronizeSingleMotionState(btRigidBody* body)
{
	...
	if (body->getMotionState() && !body->isStaticOrKinematicObject())
	{
		...
		//if (body->getActivationState() != ISLAND_SLEEPING)
		{
			...
			//if (m_is_periodic[0] || m_is_periodic[1] || m_is_periodic[2])
			//{

			//	for (int i = 0; i < 3; i++)
			//	{

			//		if (interpolatedPosition[i] >= m_mx_periodic[i])
			//		{
			//			interpolatedPosition[i] = interpolatedPosition[i] - m_mx_periodic[i] + m_mi_periodic[i];
			//		}
			//		else
			//		{
			//			if (interpolatedPosition[i] < m_mi_periodic[i])
			//			{
			//				interpolatedPosition[i] = interpolatedPosition[i] + m_mx_periodic[i] - m_mi_periodic[i];
			//			}
			//		}
